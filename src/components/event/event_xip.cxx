...
#include <iostream>
#include <stb/components/event/event_xip.h>
...
//    if(!starlight)
 ...
 //   }
   ...
   // stb::Kernel::getInstance()->getSceneManager()->setTrackerSource(otSource);
	...
	EventXIPBus::getSingleton().setEventInstance(this);
...
void
Event_XIP::setParameter(std::string key, std::string value)
{
    if(key=="configFile")
    {
        configFile=value;
    }
	else if((key.find("sinkName",0)!=std::string::npos)|(key.find("ovSinkName",0)!=std::string::npos))
...
SoTrakEngine*
Event_XIP::createSoTrakEngine()
{
    return new SoTrakEngine();
}

SoMultimodalEngine*
Event_XIP::createSoMultimodalEngine()
{
	return new SoMultimodalEngine();
}

SoTrackedArtifactKit*
Event_XIP::createSoTrackedArtifactKit()
{
...
SoOpenTrackerSource*
Event_XIP::createSoOpenTrackerSource()
{
    return new SoOpenTrackerSource();
}

SoEventGroup*
Event_XIP::createSoEventGroup()
{
    return new SoEventGroup();
}

SoEventKit*
Event_XIP::createSoEventKit()
{
    return new SoEventKit();
}

SoEventNode*
Event_XIP::createSoEventNode()
{
    return new SoEventNode();
}

SoEventSeparator*
Event_XIP::createSoEventSeparator()
{
    return new SoEventSeparator();
}

SoEventSource*
Event_XIP::createSoEventSource()
{
    return new SoEventSource();
}

SoInputEvent*
Event_XIP::createSoInputEvent()
{
    return new SoInputEvent();
}

SoEventRoot*
Event_XIP::createSoEventRoot()
{
    return new SoEventRoot();
}

SoEventAction*
Event_XIP::createSoEventAction()
{
    return new SoEventAction();
}

EventXIPBus&
Event_XIP::getEventBus()
{
    return EventXIPBus::getSingleton();
}


/*void
Event::vu_init(const openvideo::Buffer& frame, std::string *givenSinkName)
{
}
