...
#include "StbViewer/guiDefines.h"
...
SoDisplay::initClass()
{	
	SO_NODE_INIT_CLASS(SoDisplay, SoNode, "SoNode");
}
...
: viewer(NULL)
{
    ...
    SO_NODE_ADD_FIELD(updateRate, (0)); 
...
SoDisplay::~SoDisplay()
{
	...
	//tel stbViewer to remove this sodisplay from it's 'displayList'
	StbViewer::removeSoDisplay(this);
}
...
SoDisplay::createViewer()
{
	...
	//								|
	//          |------------|--------
	//         examCam    trackedItemsRoot		
	//               		 |
	//					|----------
	//
	//
	displayRoot=new SoSeparator();
	trackedItemsRoot=new SoSeparator();
	examCam= new SoPerspectiveCamera();
	displayRoot->addChild(examCam);
	displayRoot->addChild(trackedItemsRoot);
	////////////////////////////////////////
	...
	{
		if(stbCameraList[i]!=NULL && stbCameraList[i]->isOfType(SoStbCamera::getClassTypeId()))
		{
			SoStbCamera *stbCamera=(SoStbCamera *)stbCameraList[i];

			///////////////////////////////////////////////////////////////////
			///////////// Set reference camera's parameter to match with the OffAxisCamera
			///////////// of the first StbCamera ///////////////////
			if(	i==0 
				&& stbCamera->getCamera()
...
void 
SoDisplay::addTrackedItem(SoNode* trackedItem)
{
	this->trackedItemsRoot->addChild(trackedItem);
}
...
SoDisplay::readInstance(SoInput *in, unsigned short flags)
{
    ...
    if(updateRate.getValue()>0){
		startUpdate(updateRate.getValue());
	}
...
void 
SoDisplay::startUpdate(int framesPerSecond)
{
	if(!isTimerActive){
		timer= new SoTimerSensor;
		timer->setFunction(SoDisplay::timerCB);
		timer->setData( displayRoot );
		timer->setInterval( SbTime(1.0/((float)framesPerSecond))); 
		timer->schedule();
		isTimerActive=true;
	}
	else if(framesPerSecond > updateRate.getValue())
	{
		updateRate.setValue(framesPerSecond);
		timer->reschedule(SbTime(1.0/((float)framesPerSecond)));
...
void 
SoDisplay::timerCB( void * data, SoSensor * sensor )
{
    if(data==NULL) return;

	
    SoSeparator* displaysRoot = (SoSeparator*) data;
    displaysRoot->touch();
}
